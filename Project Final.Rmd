---
title: "STAT3014/STAT3914 Major Project Report"
author: "Dong Luo(480545307), Steph Standfort(460329297), and Anqi Chi(460204008)"
...
var.tab = kable_styling(kable(main.var, booktabs = TRUE, caption ="\\label{vartab}Description of Variables")%>% column_spec(1, bold = T, color = "black"),latex_options = "hold_position", position = "center")
```
...
## Research Question 1 : Popular Diet Types 
...
kable_styling(kable(
  ...
  row.names = F,
  booktabs=TRUE,
  caption = "\\label{diet_prop}Proportion of the top 6 diet types"
  ),
  latex_options = 'hold_position',
  position = "center")
```

Table \ref{diet_prop} shows the sample prorpotions of the six most popular diet types.  
...
## Research Question 2 : Predicting Obesity
...
rownames(coef.table1)<-c('(Intercept)','BMR','Energy Intake','Total mins spent sedentary','Sex','Whether currently on a diet','Carbohydrate diet','Fat diet','Protein diet')
kable_styling(kable(coef.table1,booktabs=TRUE,escape = FALSE,
            caption = '\\label{logistic}Estimated Coefficients of the Logistic Regression')%>% column_spec(1, bold = T, color = "black")%>%kable_styling(latex_options='hold_position',position='center'))
```
...
## Research Question 3 : Comparing Demographics Between Diets
...
Prior to beginning model selection, assumptions for normality of errors and homogeneity of variance were assessed using Q-Q plots and box plots of residuals, and a residual versus fitted plot was used visually assess homogeneity of variance and independence. An example of the graphs used has been provided in Figure \ref{diag} showing the diagnostic plots for the dependent variable BMR, excluding the boxplot of residuals. For variables BMR and sedentary the normality assumption was not met. We decided to apply a log transformation to BMR and a square root transformation to minutes sedentary to satisfy normality. 
...
```{r echo=FALSE, fig.cap='\\label{diag}Model Diagnostics',fig.height=2,fig.pos="H",fig.show='hold',fig.fullwidth=TRUE}
par(mfrow=c(1,3))
...
A 5% significance level was chosen as a threshold for the inclusion of the model variables. The resulting "best" models explaining each of the dependent variables is shown in Table \ref{model_selection}.
...
mod.table = kable_styling(kable(model, booktabs = TRUE, caption ="\\label{model_selection}Best models from model selection")%>% column_spec(1, bold = T, color = "black"),
...
```{r, echo = F}
#demographic table
result.tab = kable_styling(kable(result, booktabs = TRUE, caption ="\\label{demo8}Demographics for 8 key variables")%>% column_spec(1, bold = T, color = "black"),
              latex_options = "hold_position", position = "center")
```
